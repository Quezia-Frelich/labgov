export const data_palestrantes_2024 = [
  {
    profile: "/summit-2024/thais-nahas.png",
    fullName: "Thais Nahas​",
    position: "CEO - Intelink Mercado e Inovação / CEO - UT-Uban Tech ​",
    description:
      "Atua como Diretora da Vertical de Cidades Inteligentes da ACATE (Associação Catarinense de Tecnologia), conectando empresas, governos, universidades e instituições para impulsionar a inovação e o desenvolvimento urbano.​ Especialista em Living Labs, laboratórios vivos e ambientes de experimentação de tecnologias em contextos reais, possui ampla experiência na articulação de ecossistemas de inovação, no desenvolvimento de programas de inovação aberta e na aproximação entre setor público e privado para acelerar a adoção de soluções inovadoras.",
    instagram: "https://www.instagram.com/thaisnahas/",
    linkedin: "https://www.linkedin.com/in/thaisnahas",
  },
  {
    profile: "/summit-2024/Rodrigo Bianchetti​.png",
    fullName: "Rodrigo Bianchetti​​",
    position: "CEO - LAB OF CODES​​",
    description:
      "Consultor de Inovação e Transformação Digital para o Setor Público e CEO da LAB OF CODES, startup especializada em inovação, transformação digital e governança digital para o setor público. Graduado em Ciências da Computação, com pós-graduação em Desenvolvimento de Aplicações WEB e especialização em Gestão de Pessoas, possui certificação PMP e mais de 30 anos de experiência em projetos públicos no Brasil, América Central e EUA. Atua com consultoria, capacitação e mentoria, aplicando metodologias de transformação digital e inovação.​",
   
    linkedin: "https://www.linkedin.com/in/rodrigo-bianchetti/",
  },
]