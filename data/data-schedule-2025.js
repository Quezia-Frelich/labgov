export const data_schedule_2025 = [
  {
    title: "Abertura do CREA Summit 2025",
    date: "25/07",
    scenery: "plenária",
    time: "19:00",
    duration: "40",
    speakers: [
      {
        profile: "/summit-2024/Kita Xavier.jpeg",
        fullName: "Kita Xavier",
        company: "Presidente Crea-SC",
      },
      {
        profile: "/summit-2024/Vinicius Marchese.jpg",
        fullName: "Vinicius Marchese",
        company: "Presidente CONFEA",
      },
      {
        profile: "/summit-2025/joel-kruger.png",
        fullName: "Joel Krüger",
        company: "Presidente MÚTUA Nacional",
      },
      {
        profile: "/summit-2025/Juliana-Pavan.png",
        fullName: "Juliana Pavan",
        company: "Prefeita Balneário Camboriú",
      },
      {
        profile: "/summit-2025/Marilisa-Boehm.png",
        fullName: "Marilisa Boehm",
        company: "Vice-governadora do Estado de Santa Catarina",
      },
      {
        profile: "/summit-2025/Carlos-Humberto.png",
        fullName: "Carlos Humberto",
        company: "Deputado ALESC",
      },
      {
        profile: "/summit-2025/Paulinha.png",
        fullName: "Paulinha",
        company: "Deputada ALESC",
      },
    ],
  },
  {
    title: "Assinatura Convênio com a Defesa Civil",
    date: "25/07",
    scenery: "plenária",
    time: "19:40",
    duration: "10",
    speakers: [
      {
        profile: "/summit-2024/Kita Xavier.jpeg",
        fullName: "Kita Xavier",
        company: "Presidente Crea-SC",
      },
      {
        profile: "/summit-2025/Mario-Hildebrandt.png",
        fullName: "Mário Hildebrandt",
        company: "Secretário de estado da proteção e defesa civil",
      },
    ],
  },
  {
    title: "Assinatura Acordo de Cooperação Técnica com IEL/SC",
    date: "25/07",
    scenery: "plenária",
    time: "19:50",
    duration: "10",
    speakers: [
      {
        profile: "/summit-2024/Kita Xavier.jpeg",
        fullName: "Kita Xavier",
        company: "Presidente Crea-SC",
      },
      {
        profile: "/summit-2025/Mario-Aguiar.png",
        fullName: "Mario Cezar de Aguiar",
        company: "Presidente FIESC",
      },
    ],
  },
  {
    title: "Assinatura Convênio com ANE",
    date: "25/07",
    scenery: "plenária",
    time: "20:00",
    duration: "10",
    speakers: [
      {
        profile: "/summit-2024/Kita Xavier.jpeg",
        fullName: "Kita Xavier",
        company: "Presidente Crea-SC",
      },
      {
        profile: "/summit-2025/Mario-Menel.png",
        fullName: "Mario Menel",
        company: "Presidente ANE",
      },
    ],
  },
  {
    title: "Apresentação de Resultados do 15º Congresso Estadual de Profissionais",
    date: "25/07",
    scenery: "plenária",
    time: "20:10",
    duration: "15",
    speakers: [
      {
        profile: "/summit-2024/Kita Xavier.jpeg",
        fullName: "Kita Xavier",
        company: "Presidente Crea-SC",
      },
      {
        profile: "/summit-2025/felipe-penter.png",
        fullName: "Felipe Penter",
        company: "Chefe de Gabinete Crea-SC",
      },
      {
        profile: "/summit-2025/Delton Batista.png",
        fullName: "Delton Batista",
        company: "Sistema LIDE RS SC",
      },
    ],
  },
  {
    title: "Painel LIDE",
    date: "25/07",
    scenery: "plenária",
    time: "20:25",
    duration: "60",
    speakers: [
      {
        profile: "/summit-2025/Ricardo Faria.png",
        fullName: "Ricardo Farias",
        company: "“Rei do Ovo”",
      },
      {
        profile: "/summit-2024/Delton Batista.png",
        fullName: "Delton Batista",
        company: "Sistema LIDE RS SC",
      },
    ],
  },
  {
    title: "Encerramento e Networking",
    date: "25/07",
    scenery: "plenária",
    time: "21:25",
    duration: "",
    speakers: [],
  },


  {
    title: "Abertura da Feira Tecnológica",
    date: "26/07",
    scenery: "",
    time: "09:00",
    duration: "",
    speakers: [],
  },
  {
    title: "Cenários da Indústria Catarinense",
    date: "26/07",
    scenery: "plenária",
    time: "09:00",
    duration: "40",
    speakers: [
      {
        profile: "/summit-2025/Jose Eduardo Fiates.png",
        fullName: "José Eduardo Fiates",
        company: "IEL/SC",
      },
    ],
    firstSimultaneus: true,
  },
  {
    title: "Construindo Cidades para Pessoas: Como a Engenharia e Design Urbano Criam Espaços com Propósito",
    date: "26/07",
    scenery: "tech 1",
    time: "09:00",
    duration: "40",
    speakers: [
      {
        profile: "/summit-2025/dilnei-bittencourt.png",
        fullName: "Dilnei Silva Bittencourt",
        company: "Multilog / Hurbana",
      },
    ],
  },
  {
    title: "Formação de engenheiros: tendências e desafios",
    date: "26/07",
    scenery: "tech 2",
    time: "09:00",
    duration: "40",
    speakers: [
      {
        profile: "/summit-2025/Carlos-Schneider.png",
        fullName: "Carlos Alberto Schneider",
        company: "ANE",
      },
      {
        profile: "/summit-2025/claudio-jacoski.png",
        fullName: "Claudio Jacoski",
        company: "Reitor Unochapecó / Presidente ABRUC",
      },
      {
        profile: "/summit-2025/modesto-h-ferrer.png",
        fullName: "Modesto H Ferrer",
        company: "Professor UFSC Campus Joinville",
      },
    ],
  },
  {
    title: "Engenharias em Transformação: Desafios e Oportunidades na Nova Era da Empregabilidade",
    date: "26/07",
    scenery: "plenária",
    time: "09:45",
    duration: "40",
    speakers: [
      {
        profile: "/summit-2025/Jose Eduardo Fiates.png",
        fullName: "José Eduardo Fiates",
        company: "IEL/SC",
      },
      {
        profile: "/summit-2025/fernando-farina.png",
        fullName: "Fernando Farina",
        company: "Whirlpool",
      },
      {
        profile: "/summit-2025/andre-lima.png",
        fullName: "André Ribeiro de Souza Lima",
        company: "Tupy",
      },
      {
        profile: "/summit-2025/Emerson-Miranda.png",
        fullName: "Emerson da Silva Miranda",
        company: "Schulz",
      },
    ],
    firstSimultaneus: true,
  },
  {
    title: "Startups na energia e engenharia, oportunidades e sinergias",
    date: "26/07",
    scenery: "tech 1",
    time: "09:45",
    duration: "40",
    speakers: [
      {
        profile: "/summit-2025/giovane-rosa.png",
        fullName: "Giovane Rosa",
        company: "Gás Orgânico",
      },
      {
        profile: "/summit-2025/PROF-YOGUI.png",
        fullName: "PROF YOGUI. R.",
        company: "Blue Rock Brasil",
      },
      {
        profile: "/summit-2025/Annelise-Bittencourt.png",
        fullName: "Annelise Anderson Bittencourt",
        company: "GE VERNOVA",
      },
    ],
  },
  {
    title: "Liderar com lógica e com alma, projetando resultados por meio de pessoas",
    date: "26/07",
    scenery: "tech 2",
    time: "09:45",
    duration: "40",
    speakers: [
      {
        profile: "/summit-2025/Patricia-Gomes.png",
        fullName: "Patricia Gomes",
        company: "Athens Gestão e Pessoas",
      },
    ],
    lastEvent: true,
  },
  {
    title: "Inovação: o futuro das cidades e a engenharia como agente de transformação",
    date: "26/07",
    scenery: "plenária",
    time: "10:30",
    duration: "35",
    speakers: [
      {
        profile: "/summit-2025/Vinicius Marchese.png",
        fullName: "Vinicius Marchese",
        company: "Presidente CONFEA",
      },
    ],
    lastEvent: true,
  },
  {
    title: "Liderança feminina no setor público ",
    date: "26/07",
    scenery: "plenária",
    time: "11:10",
    duration: "30",
    speakers: [
      {
        profile: "/summit-2025/Marilisa-Boehm.png",
        fullName: "Marilisa Boehm",
        company: "Vice-governadora do Estado de Santa Catarina",
      },
      {
        profile: "/summit-2025/maryanne-mattos.png",
        fullName: "Maryanne Mattos",
        company: "Vice Prefeita de Florianópolis",
      },
      {
        profile: "/summit-2025/kamila-silva.png",
        fullName: "Kamila Rodrigues da Silva",
        company: "1ª Vice-Presidente CREA-SC",
      },
    ],
    lastEvent: true,
  },
  {
    title: "MÚTUA",
    date: "26/07",
    scenery: "plenária",
    time: "11:45",
    duration: "20",
    speakers: [
      {
        profile: "/summit-2025/joel-kruger.png",
        fullName: "Joel Krüger",
        company: "Presidente MÚTUA Nacional",
      },
    ],
    isLunchBreak: true,
  },
  {
    title: "Obras Públicas Paralizadas: Desafios e Soluções",
    date: "26/07",
    scenery: "plenária",
    time: "14:00",
    duration: "45",
    speakers: [
      {
        profile: "/summit-2025/mario mota.png",
        fullName: "Mario Motta",
        company: "Deputado Estadual",
      },
      {
        profile: "/summit-2025/Everton William.png",
        fullName: "Everton William Tischer",
        company: "TJSC",
      },
      {
        profile: "/summit-2025/matheus-brighenti.png",
        fullName: "Matheus Lapolli Brighenti",
        company: "TCE/SC",
      },
      {
        profile: "/summit-2025/Fernando Schmidt.png",
        fullName: "Fernando Schmit",
        company: "Nova Participações",
      },
    ],
    firstSimultaneus: true,
  },
  {
    title: "Oportunidades para o Agro Catarinense",
    date: "26/07",
    scenery: "tech 1",
    time: "14:00",
    duration: "45",
    speakers: [
      {
        profile: "/summit-2025/dirceu-leite.png",
        fullName: "Dirceu Leite",
        company: "Presidente EPAGRI",
      },
      {
        profile: "/summit-2025/Jose-Padilha.png",
        fullName: "José Almery Padilha",
        company: "OCESC",
      },
      {
        profile: "/summit-2025/felipe-penter.png",
        fullName: "Felipe Penter",
        company: "Chefe de Gabinete ",
      },
    ],
  },
  {
    title: "Atuação na Gestão de Desastres: Cooperação entre CREA-SC e Defesa Civil de Santa Catarina",
    date: "26/07",
    scenery: "tech 2",
    time: "14:00",
    duration: "45",
    speakers: [
      {
        profile: "/summit-2025/matheus-flach.png",
        fullName: "Matheus Klein Flach",
        company: "Sec. de Estado da Proteção e Defesa Civil",
      },
      {
        profile: "/summit-2025/vitor-muller.png",
        fullName: "Vitor Müller",
        company: "Diretor Crea-SC",
      },
      {
        profile: "/summit-2025/Fernanda-Vanhoni.png",
        fullName: "Fernanda Vanhoni",
        company: "Diretora Crea-SC",
      },
    ],
  },
  {
    title: "Inovação na Segurança Nacional: Os desafios para os engenheiros do amanhã",
    date: "26/07",
    scenery: "plenária",
    time: "14:55",
    duration: "45",
    speakers: [
      {
        profile: "/summit-2025/Erick Galante.png",
        fullName: "Erick Galante",
        company: "Chefe Agencia de Gestão e Inovação do Exercito",
      },
      {
        profile: "/summit-2025/Vinicios Bandeira.png",
        fullName: "Vinícios Bandeira",
        company: "Gartner",
      },
      {
        profile: "/summit-2025/Rhuan-Bittencourt.png",
        fullName: "Rhuan Bittencourt",
        company: "Head de Inovação CREA-SC",
      },
    ],
    firstSimultaneus: true,
  },
  {
    title: "Cidades Inteligentes na Prática: Soluções que Funcionam",
    date: "26/07",
    scenery: "tech 1",
    time: "14:55",
    duration: "45",
    speakers: [
      {
        profile: "/summit-2025/cibele.png",
        fullName: "Cibele Assmann",
        company: "Secretaria de Planejamento Urbano de Florianópolis",
      },
      {
        profile: "/summit-2025/Renan S. Antoniolli.png",
        fullName: "Renan S. Antoniolli",
        company: "Tuya Smart",
      },
      {
        profile: "/summit-2025/luiz-pellegrini.png",
        fullName: "Luiz Henrique Pellegrini",
        company: "Especialista em Smart Cities",
      },
    ],
  },
  {
    title: "Simulação computacional: tendências globais e o panorama regional",
    date: "26/07",
    scenery: "tech 2",
    time: "14:55",
    duration: "45",
    speakers: [
      {
        profile: "/summit-2025/rodrigo-ferraz.png",
        fullName: "Rodrigo Ferraz",
        company: "CTO - ESSS",
      },
    ],
  },
  {
    title: "Os impactos reais da inteligência artificial no trabalho: uma perspectiva científica e de mercado",
    date: "26/07",
    scenery: "plenária",
    time: "15:50",
    duration: "45",
    speakers: [
      {
        profile: "/summit-2025/andre-coelho.png",
        fullName: "André Coelho",
        company: "Autonomoss / Personal Tech",
      },
    ],
    firstSimultaneus: true,
  },
  {
    title: "Indústrias do Mar: Potenciais e Desafios da Economia Azul",
    date: "26/07",
    scenery: "tech 1",
    time: "15:50",
    duration: "45",
    speakers: [
      {
        profile: "/summit-2025/Celles-Matos.png",
        fullName: "Celles Regina de Matos",
        company: "Presidente CIDASC",
      },
      {
        profile: "/summit-2025/Tiago-Frigo.png",
        fullName: "Tiago Bolan Frigo",
        company: "Secretário Executivo da Aquicultura e Pesca do Estado de Santa Catarina",
      },
      {
        profile: "/summit-2025/Fabiana-Alexandre.png",
        fullName: "Fabiana Alexandre",
        company: "Gestora Estadual da Divisão de Defesa Sanitária Vegetal - CIDASC",
      },
    ],
  },
  {
    title: "Marketing para Engenheiros: Como gerar negócios",
    date: "26/07",
    scenery: "tech 2",
    time: "15:50",
    duration: "45",
    speakers: [
      {
        profile: "/summit-2025/nivaldo-maria.png",
        fullName: "Nivaldo de Maria",
        company: "BlueCalc Projetos",
      },
      {
        profile: "/summit-2025/Jeferson-cherobin1.png",
        fullName: "Jeferson Cherobin",
        company: "Fala JC e JC drones",
      },
      {
        profile: "/summit-2025/Flavio-Schafer.png",
        fullName: "Flávio Schäfer",
        company: "Superintendente Crea-SC",
      },
    ],
    lastEvent: true,
  },
  {
    title: "O potencial de geração de riqueza na economia verde: não somos somente uma colônia ambiental.",
    date: "26/07",
    scenery: "plenária",
    time: "16:40",
    duration: "50",
    speakers: [
      {
        profile: "/summit-2025/Ministro Joaquim Leite.png",
        fullName: "Joaquim Leite",
        company: "Ex Ministro do Meio Ambiente",
      },
    ],
    lastEvent: true,
  },
  {
    title: "Sorteio - Espaço MÚTUA-SC e CREDCREA",
    date: "26/07",
    scenery: "plenária",
    time: "17:30",
    duration: "10",
    speakers: [
      {
        profile: "/summit-2025/alvaro-dourado.png",
        fullName: "Álvaro Antônio Ribas Dourado",
        company: "Mutua SC",
      },
      {
        profile: "/summit-2025/nubia-luz.png",
        fullName: "Núbia F Luz",
        company: "Mutua SC",
      },
      {
        profile: "/summit-2025/roberta-anjos.png",
        fullName: "Roberta Maas dos Anjos",
        company: "Mutua SC",
      },
      {
        profile: "/summit-2025/Ronie-Constantino.png",
        fullName: "Ronie Constantino",
        company: "Central Ailos",
      },
    ],
    lastEvent: true,
  },
  {
    title: "Encerramento do evento",
    date: "26/07",
    scenery: "plenária",
    time: "17:40",
    duration: "20",
    speakers: [
      {
        profile: "/summit-2024/Kita Xavier.jpeg",
        fullName: "Kita Xavier",
        company: "Presidente Crea-SC",
      },
      
    ],
  },
];
